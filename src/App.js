...
const App = () => {
  ...
  let Submit = () => {
    ...
    let datDiff = findDays(currentDate, toDate);
    setArr([]);

    for (let i = 0; i < datDiff; i++) {
      const nextDay = new Date(currentDate);
      nextDay.setDate(currentDate.getDate() + i);
      const nextDayName = nextDay.toLocaleString("en-US", { weekday: "short" });
      console.log(nextDayName, nextDay);
      setArr((current) => {
        return [...current,
          {
  ...
  return (
    <div>
      <p className="date">
        From <input type="date" onChange={fromdateChange} />
        to <input type="date" onChange={TodateChange} />
